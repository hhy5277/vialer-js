<component class="component-login" tabindex="-1" v-on:keyup.enter="login">
    ...
    <!-- First-time login or a newly initiated session.-->
    <div v-else-if="!app.session.available.length || app.session.active === 'new'">
        ...
        <div class="buttons is-centered">
            <button v-if="app.session.available.length" :disabled="user.status === 'loading'" type="button" class="button cf" @click="selectSession()">{{$t('change session')}}</button>
